...
if($_SESSION["userType"] == 1){
 $wh = "1=1 and userType = 2";
}else{
  $wh = "1=1  and userType = 2 and  userID = '".$_SESSION["sys_id"]."' ";
}
$orderby = "order by userID DESC";
// $limit =" LIMIT ".$goto ." , ".$page_size ;
echo $sql = "select ".$field." from ".$table." where ".$wh ." ".$orderby;
                        ...
                        while ($rec = $db->db_fetch_array($query)) {
                          ...
                          $edit = ' <a style="'.chk_role($page_key,'isEdit').'" class="btn bg-orange btn-xs waves-effect"  onClick="editData('.$rec['userID'].');">'.$img_edit.'</a>';
                                ...
                                <tr>
                                  ...
                                  <td style="text-align: left;"><?php echo $rec['userCode'];?></td>
                                  <td style="text-align: left;"><?php echo $rec['firstname'];?></td>
                                  <td style="text-align: left;"><?php echo $rec['lastname'];?></td>
                                  <td style="text-align: left;"><?php echo $rec['mobile'];?></td>
                                  <td style="text-align: left;"><?php echo $arr_userStatus[$rec['userStatus']];?></td>
                                  <td><?php 
                                  if ($rec['activeStatus'] == 1) { ?>
